...
import (
	"encoding/json"
	"fmt"
	"github.com/Transfer_HyperledgerFabric/chaincode/plaintext_equipment_cc/lib"
...
func putZSGCPlainEquipment(stub shim.ChaincodeStubInterface, input_asset lib.Input_Plain_equipment, StatusCode string, FunctionPath string, OperatingContract string) ([]byte, bool) {

	var asset lib.Plain_equipment
...
func ZSGCPlainEquipment(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	if len(args) < 1 {
		return shim.Error("The number of args is incorrect.")
	}

	var input_plain lib.Input_Plain_equipment
	err := json.Unmarshal([]byte(args[0]), &input_plain)
	if err != nil {
		return shim.Error("Error occur when unmarshal the bytes information to struct.")
	}

	addAsset, bl := putZSGCPlainEquipment(stub, input_plain, args[2], args[3], args[4])
	if !bl {
...
func QueryPlainEquipmentByHash(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	if len(args) != 1 {
		return shim.Error("The number of args is incorrect")
	}
